...
<div class="container-fluid" x-data="{
    ...
    governateFilter: '',
    statusFilter: '',
    filterLocations() { return; }
}" x-init="
    $watch(['searchQuery', 'cityFilter', 'governateFilter', 'statusFilter'], () => filterLocations())
">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3 animate__animated animate__fadeInDown">
        <div>
            <h4 class="mb-1 fw-bold text-dark">قائمة الأماكن</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="#" class="text-muted">الشحن</a></li>
                    <li class="breadcrumb-item active text-primary">الأماكن</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('shipping.locations.create') }}"
            ...
            <span class="d-none d-sm-inline">إضافة مكان</span>
        ...
        <div class="row g-3 align-items-center">
            ...
            <div class="col-6 col-md-2">
                <select class="form-select filter-select" x-model="cityFilter">
                    ...
                    @foreach($locations->pluck('city')->unique('id') as $city)
            ...
            <div class="col-6 col-md-2">
                <select class="form-select filter-select" x-model="governateFilter">
                    ...
                    @foreach($locations->pluck('city.governate')->unique('id') as $gov)
            ...
            <div class="col-12 col-md-2 text-md-end">
                <button class="btn btn-outline-secondary rounded-pill w-100" @click="searchQuery = cityFilter = governateFilter = statusFilter = ''">
        ...
        @forelse($locations as $location)
            <div class="col-12 col-sm-6 col-lg-4 col-xl-3"
                x-show="
                    ...
                    (!cityFilter || {{ $location->city_id }} == cityFilter) &&
                    (!governateFilter || {{ $location->city->governate_id }} == governateFilter) &&
                    (statusFilter === '' || {{ $location->is_active }} == statusFilter)
                "
                ...
                <div class="location-card">
                    <div class="p-3 p-md-4 d-flex gap-3 flex-grow-1">
                        ...
                        <div class="flex-grow-1 overflow-hidden">
                            <h6 class="mb-1 fw-bold text-dark text-truncate">{{ $location->name }}</h6>
                            <p class="mb-2 text-muted small">
                                <span class="city-badge">{{ $location->city->name }}</span>
                                <span class="governate-badge ms-1">{{ $location->city->governate->name }}</span>
                            </p>
                            <div class="d-flex gap-2">
                                <span class="badge {{ $location->is_active ? 'bg-success' : 'bg-secondary' }} rounded-pill small">
                                    {{ $location->is_active ? 'نشط' : 'معطل' }}
                                </span>
                                @if($location->shippingRates->count())
                                    <span class="badge bg-info rounded-pill small">
                                        {{ $location->shippingRates->count() }} أسعار
                                    </span>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="card-footer bg-light border-0 p-3 d-flex gap-2">
                        <a href="{{ route('shipping.locations.show', $location) }}" class="action-btn btn-outline-primary" title="عرض">
                            <i class='bx bx-show'></i>
                        </a>
                        <a href="{{ route('shipping.locations.edit', $location) }}" class="action-btn btn-outline-warning" title="تعديل">
                            <i class='bx bx-edit'></i>
                        </a>
                        <form action="{{ route('shipping.locations.destroy', $location) }}" method="POST" style="display:inline;">
                            @csrf @method('DELETE')
                            <button type="submit" class="action-btn btn-outline-danger" title="حذف"
                                onclick="return confirm('هل أنت متأكد من حذف {{ addslashes($location->name) }}؟')">
        ...
        @empty
            <div class="col-12">
                <div class="empty-state animate__animated animate__fadeIn">
                    <i class='bx bx-map'></i>
                    <h5 class="mb-2">لا توجد أماكن</h5>
                    <p class="text-muted mb-3">ابدأ بإضافة مكان جديد</p>
                    <a href="{{ route('shipping.locations.create') }}" class="btn btn-primary rounded-pill px-4">
                        <i class='bx bxs-plus-circle'></i> إضافة مكان
                    </a>
                </div>
            </div>
        @endforelse
