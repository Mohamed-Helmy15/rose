{{-- resources/views/dashboard/shipping/cities/index.blade.php --}}
@extends('layouts/contentNavbarLayout')

@section('title', 'المدن - زهور')
...
@section('content')
<div class="container-fluid" x-data="{ searchQuery: '', governateFilter: '' }" x-init="$watch('searchQuery', () => filterCities()); $watch('governateFilter', () => filterCities());">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3 animate__animated animate__fadeInDown">
        <div>
            <h4 class="mb-1 fw-bold text-dark">قائمة المدن</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="#" class="text-muted">الشحن</a></li>
                    <li class="breadcrumb-item active text-primary">المدن</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('shipping.cities.create') }}"
            ...
            <span class="d-none d-sm-inline">إضافة مدينة</span>
        ...
        <div class="row g-3 align-items-center">
            ...
            <div class="col-6 col-md-3">
                <select class="form-select filter-select" x-model="governateFilter">
                    ...
                    @foreach($cities->pluck('governate')->unique('id') as $governate)
                        <option value="{{ $governate->id }}">{{ $governate->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-12 col-md-1 text-md-end">
                <button class="btn btn-outline-secondary rounded-pill w-100" @click="searchQuery = ''; governateFilter = ''">
        ...
        @forelse($cities as $city)
            <div class="col-12 col-sm-6 col-lg-4 col-xl-3"
                x-show="
                    ...
                    (!governateFilter || {{ $city->governate_id }} == governateFilter)
                "
                ...
                <div class="city-card">
                    <div class="p-3 p-md-4 d-flex gap-3 flex-grow-1">
                        ...
                        <div class="flex-grow-1 overflow-hidden">
                            <h6 class="mb-1 fw-bold text-dark text-truncate">{{ $city->name }}</h6>
                            <p class="mb-2 text-muted small">
                                <span class="governate-badge">{{ $city->governate->name }}</span>
                            </p>
                            <div class="d-flex gap-2">
                                <span class="badge {{ $city->is_active ? 'bg-success' : 'bg-secondary' }} rounded-pill small">
                                    {{ $city->is_active ? 'نشط' : 'معطل' }}
                                </span>
                                @if($city->locations->count())
                                    <span class="badge bg-info rounded-pill small">
                                        {{ $city->locations->count() }} أماكن
                                    </span>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="card-footer bg-light border-0 p-3 d-flex gap-2">
                        <a href="{{ route('shipping.cities.show', $city) }}" class="action-btn btn-outline-primary" title="عرض">
                            <i class='bx bx-show'></i>
                        </a>
                        <a href="{{ route('shipping.cities.edit', $city) }}" class="action-btn btn-outline-warning" title="تعديل">
                            <i class='bx bx-edit'></i>
                        </a>
                        <form action="{{ route('shipping.cities.destroy', $city) }}" method="POST" style="display:inline;">
                            @csrf @method('DELETE')
                            <button type="submit" class="action-btn btn-outline-danger" title="حذف"
                                onclick="return confirm('هل أنت متأكد من حذف {{ addslashes($city->name) }}؟')">
        ...
        @empty
            <div class="col-12">
                <div class="empty-state animate__animated animate__fadeIn">
                    <i class='bx bx-map'></i>
                    <h5 class="mb-2">لا توجد مدن</h5>
                    <p class="text-muted mb-3">ابدأ بإضافة مدينة جديدة</p>
                    <a href="{{ route('shipping.cities.create') }}" class="btn btn-primary rounded-pill px-4">
                        <i class='bx bxs-plus-circle'></i> إضافة مدينة
                    </a>
                </div>
            </div>
        @endforelse
