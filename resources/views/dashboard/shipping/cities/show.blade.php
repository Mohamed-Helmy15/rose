{{-- resources/views/dashboard/shipping/cities/show.blade.php --}}
@extends('layouts/contentNavbarLayout')

@section('title', '{{ $city->name }} - زهور')
...
@section('content')
    <div class="container-fluid animate__animated animate__fadeIn">
        <h4 class="py-3 mb-4 animate__animated animate__fadeInDown">
            <span class="text-muted fw-light">الشحن / المدن /</span> {{ $city->name }}
        </h4>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3 animate__animated animate__fadeInRight">
                        <div class="section-content">
                            <h6 class="section-title">المحافظة</h6>
                            <p>{{ $city->governate->name }}</p>
                        </div>
                    </div>
                    <div class="col-md-6 mb-3 animate__animated animate__fadeInLeft">
                        <div class="section-content">
                            ...
                            <p><span
                                    class="badge {{ $city->is_active ? 'bg-success' : 'bg-secondary' }}">{{ $city->is_active ? 'نشط' : 'معطل' }}</span>
                            </p>
                        </div>
                    </div>
                    <div class="col-md-12 mb-3 animate__animated animate__fadeInUp">
                        <div class="section-content">
                            <h6 class="section-title">الأماكن ({{ $city->locations->count() }})</h6>
                                        ...
                                        <tbody>
                                            @foreach ($city->locations as $loc)
                                                <tr>
                                                    <td>{{ $loc->name }}</td>
                                                    <td><span
                                                            class="badge {{ $loc->is_active ? 'bg-success' : 'bg-secondary' }}">{{ $loc->is_active ? 'نشط' : 'معطل' }}</span>
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('shipping.locations.show', $loc) }}"
                ...
                <div class="mt-4">
                    <a href="{{ route('shipping.cities.edit', $city) }}"
                        class="btn btn-primary animate__animated animate__pulse">تعديل</a>
                    <a href="{{ route('shipping.cities.index') }}"
                        class="btn btn-secondary animate__animated animate__pulse">رجوع</a>
                </div>
            </div>
        </div>
    </div>
