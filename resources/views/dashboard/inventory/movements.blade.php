...
<div class="container-fluid" x-data="{ searchQuery: '', typeFilter: '' }">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3 animate__animated animate__fadeInDown">
        <div>
            <h4 class="mb-1 fw-bold text-dark">حركات المخزون</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active text-primary">حركات</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="card mb-4 p-3 p-md-4 animate__animated animate__fadeInUp">
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 position-relative">
                ...
                <input type="text" class="form-control search-input" placeholder="ابحث بالسبب أو المنتج..."
                       x-model="searchQuery" @input.debounce.300ms="searchQuery = $event.target.value">
            </div>
            <div class="col-6 col-md-3">
                <select class="form-select filter-select" x-model="typeFilter">
                    ...
                    @forelse($movements as $movement)
                        <tr x-show="
                         ...
                         x-transition:enter="animate__animated animate__fadeInUp">
                            <td>{{ $movement->created_at->format(settings('date_format', 'd/m/Y H:i')) }}</td>
                            <td>{{ $movement->product->name }}</td>
                            <td>
                                <span class="badge {{ $movement->type == 'in' ? 'bg-success' : ($movement->type == 'out' ? 'bg-danger' : 'bg-warning') }} type-badge">
                                    {{ trans('inventory.types.' . $movement->type) }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center empty-state">
                                ...
                                <h5 class="mb-2">لا توجد حركات</h5>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
