...
@section('page-style')
    ...
    <style>
        ...
        .form-control, .form-select, .choices__inner { border-radius: 10px; transition: all 0.3s ease; }
        .form-control:focus, .form-select:focus, .choices__inner:focus {
            border-color: #4361ee !important; box-shadow: 0 0 0 0.2rem rgba(67,97,238,0.25) !important;
        }
        .btn-primary { background: linear-gradient(45deg, #007bff, #00c6ff); border: none; }
        .btn-primary:hover { background: linear-gradient(45deg, #0056b3, #0099cc); }
        .drop-zone {
            border: 3px dashed #4361ee; border-radius: 16px; padding: 40px; text-align: center;
            background: #f8f9ff; cursor: pointer; transition: all 0.3s;
        }
        .drop-zone:hover { background: #e0e7ff; }
        .image-preview { width: 120px; height: 120px; object-fit: cover; border-radius: 12px; border: 4px solid white; box-shadow: 0 8px 25px rgba(0,0,0,0.2); margin: 5px; }
            ...
            <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control form-control-lg" name="price" value="{{ old('price') }}" required>
                        @error('price') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">سعر التكلفة</label>
                        <input type="number" step="0.01" class="form-control" name="cost_price" value="{{ old('cost_price') }}">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">حد إعادة الطلب (تنبيه عند الوصول له)</label>
                        <input type="number" class="form-control" name="reorder_level" value="{{ old('reorder_level', 10) }}" min="0">
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        dropZone.ondrop = e => {
            e.preventDefault(); dropZone.style.background = '#f8f9ff';
