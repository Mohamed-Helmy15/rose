{{-- resources/views/dashboard/purchase_orders/edit.blade.php --}}
@extends('layouts/contentNavbarLayout')

@section('title', 'تعديل أمر شراء - زهور')
...
@section('content')
    <div class="container-fluid animate__animated animate__fadeIn">
        <h4 class="py-3 mb-4 animate__animated animate__fadeInDown">
            <span class="text-muted fw-light">أوامر الشراء /</span> تعديل أمر شراء
        </h4>

        <div class="card animate__animated animate__zoomIn">
            <h5 class="card-header bg-primary text-white py-4">تعديل أمر الشراء #{{ $purchaseOrder->id }}</h5>
                ...
                <form action="{{ route('purchase-orders.update', $purchaseOrder) }}" method="POST">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12 animate__animated animate__fadeInUp">
                            ...
                            <textarea class="form-control" rows="3" name="notes">{{ old('notes', $purchaseOrder->notes) }}</textarea>
                        </div>
                    </div>

                    <h5 class="mt-4">المنتجات</h5>
                    <div id="products-container">
                        @foreach ($selectedProducts as $index => $selected)
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-3">
                                        <input type="number" name="products[{{ $index }}][quantity]"
                                            class="form-control quantity" value="{{ $selected['quantity'] }}"
                                            min="1">
                                    </div>
                                    <div class="col-md-3">
                                        <input type="number" step="0.01" name="products[{{ $index }}][price]"
                                            class="form-control price" value="{{ $selected['price'] }}">
                                    </div>
                                    <div class="col-md-2">
                    ...
                    <div class="mt-5 d-flex gap-3 justify-content-end">
                        <button type="submit" class="btn btn-primary btn-lg px-5 animate__animated animate__pulse">
                            <i class='bx bx-check'></i> تحديث أمر الشراء
                        </button>
